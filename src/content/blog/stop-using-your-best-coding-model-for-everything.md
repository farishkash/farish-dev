...
## What this can look like in AGENTS.md

Codex can follow project-level `AGENTS.md` instructions that request delegation, which means some of this routing policy can live with the project instead of only in my head. The actual model assignment for a custom agent lives in `.codex/agents/*.toml`; `AGENTS.md` is where I tell the orchestrator when and why to use those agents.

Here’s a simplified version of the routing policy I’m describing:

```markdown
# Agent strategy

The primary agent is responsible for architecture, planning,
integration, and final review.

Use subagents deliberately. Do not use the strongest model for every
implementation step when the problem has already been reduced to a
clear specification.
...
If Luna is repeating the same hypothesis or is unable to resolve the
problem after reasonable attempts:
...
4. Ask `terra_debugger` to identify the likely root cause before making
   broad changes.
5. Escalate back to the primary agent if the problem appears architectural,
   crosses multiple systems, or requires reconsidering the implementation plan.
...
Avoid having multiple agents edit overlapping files at the same time unless
there is a clear reason to do so.
```

I’ve also kept this as a standalone [example AGENTS.md file](https://github.com/farishkash/farish-dev/blob/main/examples/sol-luna-routing/AGENTS.md) that can be copied and adapted for a project.
